...
# Parse the options
OPTIND=1
while getopts "hdDs:e:f:g:G:kK:l:H:tai:m:T:p" opt; do
    case "$opt" in
        h) usage;;
	d) dryrun=1;;
    D) debug=1;;
...
## setup the tmp dir
if [ -z $SNIC_NOBACKUP ]; then
    export SNIC_TMP=/mnt/picea/tmp
else
    export SNIC_TMP=$SNIC_NOBACKUP
    mem="mem$mem"
    memArg="-C"
fi
...
if [ $pstart -le 7 ] && [ $pend -ge 7 ]; then
    ...
    JOBCMDS+=(`prepare_$CMD \
            ...
            -J ${sname}.RNAseq.STAR \
            ${mem:+"-m" "$mem"} \
            $dep runSTAR.sh -l "$bam_memory" $star_runner_options $star $star_ref $fastq_trimmed_1 $fastq_trimmed_2 $star_options`)
    JOBIDS+=(`run_$CMD ${JOBCMDS[${#JOBCMDS[@]}-1]}`)
fi
...
if [ $pstart -le 9 ] && [ $pend -ge 9 ]; then
    ...
    JOBCMDS+=(`prepare_$CMD \
        ...
        $dep runKallisto.sh $fastq_trimmed_1 $fastq_trimmed_2 $kallisto_index $kallisto_fasta $kallisto`)
    JOBIDS+=(`run_$CMD ${JOBCMDS[${#JOBCMDS[@]}-1]}`)
fi
